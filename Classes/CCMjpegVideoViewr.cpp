...
CCMjpegVideoViewr* CCMjpegVideoViewr::create(const char* url,const Color4B& color)
{
	CCMjpegVideoViewr *viwer = new (std::nothrow) CCMjpegVideoViewr();
...
void CCMjpegVideoViewr::draw(Renderer *renderer, const Mat4& transform, uint32_t flags)
{
	if (m_recv->getRunningState() != false)
	{
		...
		_customCommand.func = CC_CALLBACK_0(CCMjpegVideoViewr::onDraw, this, transform, flags);
...
void CCMjpegVideoViewr::onDraw(const Mat4& transform, uint32_t flags)
{
	getGLProgram()->use();
...
void CCMjpegVideoViewr::pause()
{
	m_recv->closeReceiver();
	Node::pause();
}

void CCMjpegVideoViewr::resume()
{
	m_recv->openReceiver(m_url.c_str());
	Node::resume();
}

void CCMjpegVideoViewr::setTexture(Image* img)
{
	if (_texture)
	{
...
void CCMjpegVideoViewr::updateTexture(Image* image)
{
	if (image == NULL)
		return;
	if (_texture == NULL)
...
void CCMjpegVideoViewr::setContentSize(const Size & size)
{
...
bool CCMjpegVideoViewr::init(const Color4B& color)
{
...
void CCMjpegVideoViewr::updateColor()
{
