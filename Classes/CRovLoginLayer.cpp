...
////	}
//	
	auto joy = CUIJoystick::create("joystick_manuel.png", "joystick_halo.png");
	
	joy->setContentSize(cocos2d::Size(200.f, 200.f));
	joy->setPosition(visibleSize / 2);
	this->addChild(joy, 9);
	return true;
}
...
{
	Scene* sc = RovScene::createScene(_ip.c_str(),true);
	if (sc)
	{
...
{
	Scene* sc = RovScene::createScene(_ip.c_str(), false);
	if (sc)
	{
		Director::getInstance()->replaceScene(sc);
	}
